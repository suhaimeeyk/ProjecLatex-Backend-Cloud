...
app.get('/db_dataALLOwner/:users_id', (req, res) => {
    ...
    if (!users_id) {
        ...
        connection.query("SELECT  m.data_id,u.users_name, c.customer_name  , c.customer_id , DATE_FORMAT(m.data_date, '%d/%m/%Y') as data_date ,m.data_totalgallon as น้ำหนักรวมทั้งหมด , m.data_wgallon as น้ำหนักแกลลอน, m.data_disgallon as น้ำหนักหักลบแกลลอน , m.data_dryrubber as น้ำยาแห้ง , m.data_price as เปอร์เซ็น  , m.data_pricetotal as  จำนวนเงินทั้งหมด  , c.customer_name as ผู้ตัดยาง , m.data_depositprice as เงินส่วนเจ้าของสวน, m.data_shareprice as เงินส่วนคนตัด FROM db_data AS m INNER JOIN db_customer AS c ON m.data_usersid = c.customer_id INNER JOIN db_users AS u ON u.users_id = c.db_users_id WHERE m.owder_id = ? GROUP BY  m.data_id,c.customer_name, c.customer_id, m.data_date ,m.data_totalgallon, m.data_wgallon, m.data_disgallon , m.data_percent,m.data_dryrubber , m.data_price , m.data_pricetotal ,m.data_shareprice , m.data_depositprice , c.customer_name ORDER BY m.data_date DESC;", users_id, (error, results, fields) => {
            if (error) throw error;
...
app.get('/cumulative_tire_price/:users_id', (req, res) => {
    ...
    if (!db_users_id) {
        ...
        connection.query("SELECT ROUND(SUM(data_pricetotal), 2) AS ผลร่วมจ่ายค่าน้ำยางต่อวัน, u.users_name, c.customer_name, c.customer_id FROM db_data AS m INNER JOIN db_customer AS c ON m.data_usersid = c.customer_id INNER JOIN db_users AS u ON u.users_id = c.db_users_id WHERE c.db_users_id = ? GROUP BY c.customer_name, c.customer_id ORDER BY SUM(data_pricetotal) DESC;", db_users_id, (error, results, fields) => {
            if (error) throw error;
...
app.get('/cumulative_tire_price_Owner/:users_id', (req, res) => {
    ...
    if (!db_users_id) {
        ...
        connection.query("SELECT SUM(m.data_depositprice) AS ยอดเงินสะสมของเจ้าของสวน FROM db_data AS m INNER JOIN db_customer AS c ON m.data_usersid = c.customer_id INNER JOIN db_users AS u ON u.users_id = c.db_users_id WHERE m.owder_id = ?", db_users_id, (error, results, fields) => {
            if (error) throw error;
...
app.get('/cumulative_tire_price_customer_id/:customer_id', (req, res) => {
    ...
    if (!customer_id) {
        ...
        connection.query("SELECT DATE_FORMAT(data_date, '%d/%m/%Y') AS วันที่ , ROUND(SUM(data_pricetotal), 2) AS ผลร่วมจ่ายค่าน้ำยางต่อวัน, u.users_name, c.customer_name, c.customer_id FROM db_data AS m INNER JOIN db_customer AS c ON m.data_usersid = c.customer_id INNER JOIN db_users AS u ON u.users_id = c.db_users_id WHERE c.customer_id = ? GROUP BY DATE_FORMAT(data_date, '%d/%m/%Y'), c.customer_name, c.customer_id;", customer_id, (error, results, fields) => {
            if (error) throw error;
